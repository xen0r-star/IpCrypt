...
def definirMasque(classe):
    """Retourne le masque par défaut (classful) associé à la classe donnée."""
    match classe:
        case "A":
...
def create_get_mask_ui(on_back=None):
    """Fenêtre Recherche masque IP : affiche le masque classful correspondant à l'adresse saisie."""
    group_entries.clear()
    result_labels.clear()
    app = ctk.CTk()
    if ICO.exists():
    ...
    for label_text in ["Masque du réseau"]:
    ...
    def on_verify():
        """Lit les octets, détermine la classe puis affiche le masque classful correspondant."""
        octets = lire_octets(group_entries)

        if octets:
            classe = definirClasse(octets[0])
            masque = definirMasque(classe)
            result_labels["Masque du réseau"].configure(text=masque, text_color=COLORS["primary"])
        else:
            result_labels["Masque du réseau"].configure(text="IP Invalide", text_color=COLORS["danger"])
